...
func args() ([]string, []string) {
	status_codes := flag.StringArrayP("expected-status-code", "c", []string{"200"}, "Check expected status code is returned.")
	flag.Usage = func() {
		fmt.Println("Usage: ajpclient URL1 URL2 ... http://localhost:8009")
		flag.PrintDefaults()
	}
	...
	if len(*status_codes) > 1 {
		if len(*status_codes) > len(flag.Args()) {
			fmt.Println("Too many status codes given!\a")
			os.Exit(1)
		} else if len(*status_codes) < len(flag.Args()) {
			fmt.Println("Not enough status codes given!\a")
			os.Exit(1)
		}
	}

	return *status_codes, flag.Args()
}
...
func main() {
	expected_status_codes, urls := args()

	code := atou16(expected_status_codes[0])
	for i, url := range urls {
		if len(expected_status_codes) > 1 {
			code = atou16(expected_status_codes[i])
		}
...
func ajpclient(expected_status_code uint16, flag_url string) {
	var status_code int32 = -1
	...
	LoopContent:
		for {
			...
			case AJP13_SEND_HEADERS:
				...
				ajp_reader.headers()
				status_code = int32(ajp_reader.status_code)
			...
			case AJP13_END_RESPONSE:
				...
				conn.Close()
				if status_code > 0 && status_code == int32(expected_status_code) {
					return
				} else {
					fmt.Println("Unexpected status code:", status_code, "\a")
					fmt.Println("Expected status code:", expected_status_code, "")
					os.Exit(1)
				}
			default:
				break
