...
class StaticResourceHandler implements HttpHandler {
  ...
  public void handle(HttpExchange exchange) throws IOException {
    ...
    var contentType = ContentType.getFromExtension(getFileExtension(path));
    var content = IO.loadResourceFromClasspath(path);
    if (content.length == 0) {
      status = HttpStatus.NOT_FOUND;
      content = HtmlSupport.renderErrorPage(status, String.format("%s not found on this server", path));
